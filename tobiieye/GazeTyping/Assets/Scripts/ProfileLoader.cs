...
public class ProfileLoader : MonoBehaviour
{
    ...
    /// </summary>
    [SerializeField]
    public static Dictionary<string, List<string>> configMap;
    [SerializeField]
    ...
    {
        configMap = new Dictionary<string, List<string>>();
        ...
        {
            // item.value is a list now
            var itemValueList = item.Value.Deserialize<string[]>();
            foreach (string itemValue in itemValueList)
            {
                //Debug.Log(itemValue);
                if (configMap.ContainsKey(item.Key))
                {
                    Debug.LogWarning("key: " + item.Key + " already exists.");
                    configMap[item.Key].Add(itemValue);
                }
                else
                {
                    configMap.Add(item.Key, new List<string>() { itemValue });
                }
                if (letterMap.ContainsKey(itemValue))
                {
                    letterMap[itemValue].Add(item.Key[0]);
                }
                else
                {
                    letterMap[itemValue] = new List<char>();
                    letterMap[itemValue].Add(item.Key[0]);
                }
            }
        }
